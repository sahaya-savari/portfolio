    ...
    <section id="certifications" aria-label="Certifications and courses" className="py-32 px-6">
      <div className="max-w-screen-xl mx-auto">
        ...
        <div>
          ...
          <div role="tablist" className="flex overflow-x-auto hide-scrollbar gap-2 justify-start md:justify-center mb-8 px-2">
          ...
          <div className="liquid-glass rounded-3xl p-6 md:p-8 min-h-[300px]">
            {CERTIFICATIONS.map(({ title, items }) => (
              <div 
                key={title} 
                className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 transition-opacity duration-300 ${activeTab === title ? 'block opacity-100' : 'hidden opacity-0'}`}
                role="tabpanel"
              >
                {items.map((c: any) => (
